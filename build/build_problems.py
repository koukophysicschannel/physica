...
# 章 -> 分野 mapping. Provisional per spec ("要確認"); edit here and rerun the
# build script if the school's chapter/field boundaries change.
...
def main():
    ...
    # Sanity checks against spec counts.
    assert len(leadalpha) == 591, f"expected 591 leadalpha problems, got {len(leadalpha)}"
    assert len(juyomon) == 163, f"expected 163 juyomon problems, got {len(juyomon)}"

    counts = {}
    for p in leadalpha:
        counts[p["categoryKey"]] = counts.get(p["categoryKey"], 0) + 1
    assert counts.get("基礎CHECK") == 27, counts
    assert counts.get("基本例題") == 93, counts
    assert counts.get("基本問題") == 338, counts
    assert counts.get("応用問題") == 133, counts
