...
このリポジトリは自宅Macと職場Macの両方から作業される。

- **作業開始時**: 必ず `git pull` し、本ファイルの「進行中タスク」欄を確認すること。
- **作業終了時**: 必ず `push` し、「進行中タスク」欄を更新すること。
- **同一タスクを両環境で並行実装しない**。

## 進行中タスク

- **v2.13指示書**(`PHYSICA-v2.13-directive.md`)の2件目のみ残っている。
  - 問題評価システム(ratings.json + admin.html): **完了**(commit `1705d83`)。
    admin.html は https://koukophysicschannel.github.io/physica/admin.html
    (ナビ非リンク・URL直打ちのみ)。通勤評価を始める前に、まず一度アクセスして
    問題なく動くか確認してください。
  - curated型パッケージ: **保留**。元データ「2026予定表.pdf」がプロジェクトフォルダに
    見当たらない(`000 開智所沢中等教育学校/.../2026予定表.pdf` という同名ファイルは発見したが
    無関係の中学校プリントの可能性が高く、指示書が指す「2025夏 物理ペースメーカー」PDFとは
    別物と判断し使用していない)。管理者に確認待ち — 正しいPDFの所在を教えてもらい次第、着手する。
- **概要欄一括更新(YouTube、実施場所は `002 リード/999 抽出`)**: 689本中195本完了・残り494本。
  引き続き `python3 update_youtube_descriptions.py --limit 190` を日次で実行中(詳細は
  そちらのフォルダの `HANDOFF.md` 参照)。689本完了時にこのHANDOFF.mdへ追記予定。

## 完了済み(直近5件)

1. `a73d020` about.htmlの忘却曲線をKaTeXで数式組版(vendor/katex/にローカル同梱、一般形+config値埋め込みの具体形を併記、「tau(基準日数)/floor(下限)」→「τ(基準日数)/下限」に表記変更、sw.js を physica-v2.12 に更新。本番で375px幅・オンライン/オフライン両方のレンダリングを確認済み)
2. `2316ade` HANDOFF.mdに運用ルール・進行中タスク・完了済み履歴のセクションを追加
3. `5dd0a41` HANDOFF.md訂正(koukophysicschannelは個人アカウントである旨、旧URL404は実害なしと明記)+ git identityをkoukophysicschannel名義に更新 + 重複ファイル削除
4. `d96f88b` 重問「力学」の再生リストURLを設定
5. `0f4106b` 重問プレイリストの器を追加、Tier1大学名を東京科学大学(理)に変更
