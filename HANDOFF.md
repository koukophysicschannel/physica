...
- about.htmlの忘却曲線の式表示をKaTeXで数式組版に変更(コード風プレーンテキストから移行)。
  - CSS/JSはCDNではなくローカル同梱の方針で実装中(オフライン時のフォント欠落・キャッシュの
    不確実性を避けるため)。config.jsonのfloor/tauから数値を動的に埋め込む。
  - 「tau(基準日数)」「floor(下限)」の表記を「τ(基準日数)」「下限」に変更する作業も含む。
  - sw.jsバージョンアップ・本番反映・375px幅での検品・スクリーンショット報告まで未完了。

## 完了済み(直近5件)

1. `5dd0a41` HANDOFF.md訂正(koukophysicschannelは個人アカウントである旨、旧URL404は実害なしと明記)+ git identityをkoukophysicschannel名義に更新 + 重複ファイル削除
2. `d96f88b` 重問「力学」の再生リストURLを設定
3. `0f4106b` 重問プレイリストの器を追加、Tier1大学名を東京科学大学(理)に変更
4. `2d63da2` 志望ランクを8Tier制の大学選択に刷新
5. `0698369` 「上に戻る」フローティングボタンと章/分野ジャンプバーを追加
