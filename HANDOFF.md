...
- **v2.13指示書**(`PHYSICA-v2.13-directive.md`、未コミット・リポジトリ直下に配置済み)に着手中。
  1. 問題評価システム(ratings.json + admin.html): 実装中。明日朝の通勤評価開始に向けて優先。
  2. curated型パッケージ: **保留**。元データ「2026予定表.pdf」がプロジェクトフォルダに
     見当たらない(`000 開智所沢中等教育学校/.../2026予定表.pdf` という同名ファイルは発見したが
     無関係の中学校プリントの可能性が高く、指示書が指す「2025夏 物理ペースメーカー」PDFとは
     別物と判断し使用していない)。管理者に確認待ち。
